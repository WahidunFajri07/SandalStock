...
import { Pencil, Trash2, CalendarDays, FilterX, PackagePlus } from 'lucide-react'
...
export default function StockInPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/stock-in?${p}`)
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Failed to load stock-in records')
  ...
  const submit = async (e) => {
    ...
    setSaving(true)
    try {
      const res  = await fetch('/api/stock-in', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, quantity: Number(form.quantity) }) })
      const data = await res.json()
  ...
  const saveEdit = async () => {
    try {
      const res  = await fetch(`/api/stock-in/${editId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...editForm, quantity: Number(editForm.quantity) }) })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error)
  ...
  return (
    <div className="space-y-6 animate-soft-in">
      ...
      <div className="glass rounded-[2rem] p-5 sm:p-6 border-slate-200/50 dark:border-white/10 shadow-sm flex items-center gap-4 sm:gap-5">
        <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-white shadow-lg shadow-amber-500/20 shrink-0 border-2 border-white/20 dark:border-white/10">
           <PackagePlus className="w-6 h-6 sm:w-7 sm:h-7 drop-shadow-md" strokeWidth={2.5} />
        </div>
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-slate-800 dark:text-white tracking-tight">{t('stockIn')}</h1>
          <p className="text-[11px] sm:text-sm text-slate-500 dark:text-slate-400 font-medium mt-0 sm:mt-1 leading-tight sm:leading-normal">{t('stockInHistory')}</p>
        </div>
      </div>
      ...
      {isAdmin && (
        <form onSubmit={submit} className="glass rounded-[2rem] shadow-sm border-slate-200/50 dark:border-white/10 p-6 transition-soft">
          <h2 className="text-sm font-black text-slate-800 dark:text-white mb-4 uppercase tracking-wider flex items-center gap-2"><span>➕</span> {t('newStockIn')}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Field label={t('quantity')}>
              <input type="number" min="1" value={form.quantity} onChange={e => setForm(f => ({ ...f, quantity: e.target.value }))} placeholder="e.g. 24" className={inputCls} />
            </Field>
            <Field label={t('date')}>
          ...
          <button type="submit" disabled={saving}
            className="btn-elegant mt-6 px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold rounded-xl transition-all shadow-lg shadow-emerald-500/25 disabled:opacity-50">
            {saving ? t('processing') : t('recordStockIn')}
          </button>
        </form>
      )}
      ...
      <div className="glass rounded-[2rem] shadow-sm border-slate-200/50 dark:border-white/10 overflow-hidden transition-soft">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800/80">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-black text-slate-800 dark:text-white">Riwayat Stok Masuk</p>
              <p className="text-[10px] text-slate-400 font-medium">{rows ? `${rows.length} record ditemukan` : 'Memuat...'}</p>
            </div>
          </div>
              ...
              {!rows ? (
                ...
                <tr key={row.id}>
                  <td className="text-center">
                    <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-emerald-50 dark:bg-emerald-500/10 text-[10px] font-black text-emerald-600 dark:text-emerald-400">{idx + 1}</span>
                  </td>
                  ...
                  <td>
                    <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-black bg-emerald-50 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/30">
                      +{row.quantity}
                    </span>
                  </td>
      ...
      {/* Edit Modal */}
      <Modal open={editModal} onClose={() => setEditModal(false)} title={t('editStockIn')}
        footer={
          <>
